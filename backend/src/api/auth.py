...
tokens_response = list[RefreshTokenSchema | AccessTokenSchema]
...
async def sign_up(new_user: AuthUserSchema, session=Depends(get_base_session)):
    ...
    try:
        ...
        rts = auth_utils.create_token(user.email, user.created_at, "refresh")
        ats = auth_utils.create_token(user.email, user.created_at, "access")
        logger.info(f"User(email={new_user.email}) was created")
        return [
            RefreshTokenSchema(refresh_token=rts, token_type="bearer"),
            AccessTokenSchema(access_token=ats, token_type="bearer"),
        ]
...
@router.post("/sign_in_docs", status_code=200, response_model=AccessTokenSchema)
...
async def _sign_in(user_data: AuthUserSchema, session) -> tokens_response:
    ...
    rts = auth_utils.create_token(user.email, now, "refresh")
    ats = auth_utils.create_token(user.email, now, "access")
    return [
        RefreshTokenSchema(refresh_token=rts, token_type="bearer"),
        AccessTokenSchema(access_token=ats, token_type="bearer"),
    ]
...
async def refresh(
    ...
    repo = UserRepository(session)
    user = await repo.get_user(refresh_token.email)
    if user is None:
